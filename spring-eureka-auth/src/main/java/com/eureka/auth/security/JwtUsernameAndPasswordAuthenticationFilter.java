...
import com.eureka.auth.request.LoginRequest;
...
public class JwtUsernameAndPasswordAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		/*
		 * Long now = System.currentTimeMillis(); String token =
		 * Jwts.builder().setSubject(auth.getName()) // Convert to list of strings. //
		 * This is important because it affects the way we get them back in the Gateway.
		 * .claim("authorities",
		 * auth.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(
		 * Collectors.toList())) .setIssuedAt(new Date(now)).setExpiration(new Date(now
		 * + jwtConfig.getExpiration() * 1000)) // in // milliseconds
		 * .signWith(SignatureAlgorithm.HS512,
		 * jwtConfig.getSecret().getBytes()).compact();
		 * 
		 * // Add token to header response.addHeader(jwtConfig.getHeader(),
